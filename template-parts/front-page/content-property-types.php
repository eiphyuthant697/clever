					...
					<?php if ($property_types):    ?>
	                   <?php foreach ($property_types as $property_key => $property_type ) :?>
						<li <?php if ( $property_key == 0 ) { ?>class="active"<?php } ?>>
							<a href="#<?php echo esc_attr( $property_type->slug); ?>"><?php echo esc_html( $property_type->name ); ?></a>
						</li>
		              <?php endforeach;?>
	               <?php endif;?>
				...
				<div class="tab-content">
					<?php foreach ( $property_types as $property_key => $property_type ) : ?>
							...
							<?php if ( $property_type ) : ?>
								<div id="<?php echo esc_attr( $property_type->slug); ?>" class="tab<?php if ( $property_key == 0 ) { ?> active<?php } ?>">
									...
									$args = [
										...
										'tax_query'             => array(
											array(
												...
												'terms'             => $property_type->term_id,    // term id, term slug or term name
											)
										),
									...
									if ( $enable_view_all && array_key_exists( 'desktop', $enable_view_all ) ) : ?>
										<div class="d-flex justify-content-center btn-wrap">
											<span class="box-button view-all-btn"><a href="<?php echo esc_url( get_category_link( $property_type->term_id ) ); ?>"><?php esc_html_e( 'View All', 'real-home' ); ?></a></span>
										</div>
									<?php
									endif;
									?>
